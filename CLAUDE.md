...
| License | MIT | Matches all vendored upstreams (no license-mixing). |
| Distribution | Plugin **and** copy-able `skills/` folder | One-command install for most; copy path for the rest. |
| Dependencies | Vendor superpowers + karpathy (pure markdown, MIT). **Do NOT vendor gstack.** | gstack is a ~1 GB Bun/TypeScript product (compiled `browse` CLI + Chrome extension) — vendoring = forking a live product forever. |
...
MIT skill, with one mechanical transformation applied: skill cross-references are re-namespaced from
...
`/design-review`, `/autoplan`, `/ship`, `/setup-browser-cookies`). `frontend-design` (Anthropic
plugin skill) is an optional dependency, required only for UI work.

## Rules for changes

- **Never hand-edit a vendored skill.** Fixes belong upstream; pull them back with
  `scripts/sync-vendored.sh <superpowers-ref> <karpathy-ref>`, then update the version pins + date in
  `VENDORED.md` and run `scripts/lint.sh`.
...
- **Phase 0 — Preflight** — verify gstack (+ frontend-design for UI work) installed; stop with install
