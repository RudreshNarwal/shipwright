#!/usr/bin/env bash
# Bootstrap gstack — the one Shipwright dependency that can't be vendored — and everything it needs:
...
#
# Env: GSTACK_DIR (default ~/.claude/skills/gstack), SHIPWRIGHT_ASSUME_YES=1 (same as --yes).
set -euo pipefail

GSTACK_DIR="${GSTACK_DIR:-$HOME/.claude/skills/gstack}"
REPO="https://github.com/garrytan/gstack.git"

# --yes, SHIPWRIGHT_ASSUME_YES=1, or a non-interactive shell (e.g. the SessionStart hook) → no prompts.
ASSUME_YES=0
{ [ "${1:-}" = "--yes" ] || [ "${SHIPWRIGHT_ASSUME_YES:-}" = "1" ] || [ ! -t 0 ]; } && ASSUME_YES=1
...
echo "Shipwright :: gstack bootstrapper"
...
echo "gstack is ready. Shipwright Phases 5-6 (browser + API QA, ship) can now run."
